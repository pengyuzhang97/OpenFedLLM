...
script_args.output_dir = 'merge_fp_output'
...
def merge_with_global_dict(g_lora_model):
    if script_args.load_in_4bit:
        target_module = bnb.nn.Linear4bit
    elif script_args.load_in_8bit:
        target_module = bnb.nn.Linear8bit
...
for round in tqdm(range(fed_args.num_rounds)):
    ...
    for client in range(fed_args.num_clients):
        ...
        # reinitialize the lora structure and parameters
        ...
        # else:
        #     # TODO: assign client-specific global_dict based on global_dict_max_r
        ...
        rewrite = False
        if rewrite:
            from utils.template import get_formatting_prompts_func_for_rewrite
            rewrite_template = """ Below is an instruction that describes a task along with a reference answer. 
            Refer to the reference answer, write your own response and put your response after ### Yours.
            ### Instruction: {}
            ### Reference Answer: {}
            ### Yours:
            """
            ...
            for i in range(len(training_data)):
                ...
                temp_data = rewrite_template.format(training_data[i]['instruction'], training_data[i]['response'])
            ...
            for batch_start_idx in tqdm(range(0, len(current_data), bs)):
                batched_inputs = tokenizer(current_data[batch_start_idx:batch_start_idx+bs],
                ...
                batched_output_str = tokenizer.batch_decode(batched_outputs[:,batched_inputs.data['input_ids'].size(1):], skip_special_tokens=True)
                raw_rewrite_data.append(batched_output_str)
                # break


            # for i in tqdm(range(len(current_data))):
            #     inputs = tokenizer.encode(current_data[i], return_tensors='pt').to(model.device)
            #     outputs = model.generate(inputs, max_length=1024)
            #     outputs_str = tokenizer.decode(outputs[0], skip_special_tokens=True)
            #     raw_rewrite_data.append(outputs_str)
    ...
    global_dict = global_aggregate_hybrid(
    ...
    if (round + 1) % fed_args.save_model_freq == 0:
        ...
        model.delete_adapter('default')
        model.unload()
        print('Delete and unload lora of the global model')
